/*
 * @lc app=leetcode id=1882 lang=typescript
 *
...
//   let serverStatus = new Array(servers.length).fill(0);
...
//     // console.log("time: ", time, " current Task index: ", currentTaskIndex);
//     for (let serverIndex = 0; serverIndex < servers.length; serverIndex++) {
//       shouldBreak = time > currentTaskIndex ? false : true;
//       if (serverStatus[serverIndex] === 0 && tasks.length > 0) {
//         let currentTask = tasks.shift();
//         // console.log("Found Idle server #", serverIndex);
//         // console.log(
//         //   "Inserting task #",
//         //   currentTaskIndex,
//         //   ", processing time: ",
...
//         // );
//         currentTaskIndex++;
//         serverStatus[serverIndex] = currentTask;
//         jobQueue.push(serverIndex);

//         // console.log(
//         //   "Inserting finished. Current server status: ",
//         //   serverStatus
//         // );
...
//       //     //   console.log("All servers are busy, thread jammed.");
//       //     break;
//       //   }
//     }
...
//       }
//     }
//     // console.log("Processing finished. Current server status: ", serverStatus);
//     time++;
//   }

//   const serversPriorityListIndex = getServersPriorityListIndex(servers);
//   const ans = jobQueue.map((job) => serversPriorityListIndex[job]);

//   return ans;
// }

// function getServersPriorityListIndex(servers: number[]): number[] {
//   const list: number[] = [];
...
function assignTasks(servers: number[], tasks: number[]): number[] {
  ...
  let serversMap: number[][] = [];
  servers.forEach((server, index) => serversMap.push([server, index]));
  let idle = new MinHeap(serversMap);
  let res = tasks.slice();
  let c = 0;
  tasks.forEach((t, t_idx) => {
    c = Math.max(c, t_idx);
    while (busy.size() > 0 && busy.top()[0] <= c) {
      let newIdle = busy.pick();
      idle.add([servers[newIdle[1]], newIdle[1]]);
    }
    if (idle.size() === 0) {
      c = busy.top()[0];
      while (busy.size() > 0 && busy.top()[0] <= c) {
...
class MinHeap {
  ...
  down(i) {
    ...
    if (
      l < this.heap.length &&
      (this.heap[l][0] < this.heap[smaller][0] ||
        (this.heap[l][0] === this.heap[smaller][0] &&
          this.heap[l][1] < this.heap[smaller][1]))
    )
      smaller = l;
    if (
      r < this.heap.length &&
      (this.heap[r][0] < this.heap[smaller][0] ||
        (this.heap[r][0] === this.heap[smaller][0] &&
          this.heap[r][1] < this.heap[smaller][1]))
    )
      smaller = r;
    if (smaller !== i) {
      this.swap(smaller, i);
